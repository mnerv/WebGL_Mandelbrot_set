...
void main() {
  ...
  // Get uv coordinate and center it
  vec2 uv_or = gl_FragCoord.xy - 0.5 * u_resolution.xy;
  ...
  uv = mix(uv_or, uv, u_symmetry); // linear interpolation
  ...
  float m = sqrt(iter / u_maxIter);
  vec3 col;

  if (iter < u_maxIter)
    col = sin(u_cOffset.xyz * m * u_cOffset.w) * .5 + 0.5;
  else
    col = vec3(0);

  // float angle = atan(z.x, z.y); // -pi and pi
  // col *= vec3(smoothstep(3., 0., fracIter));

  // col *= vec3(1. + sin(angle * 2.) * .2);
